...
public class AddressSearchServiceTests
{
    ...
    public async Task ExecuteSearchAsync_ReturnsFailure_WhenQueryInvalid()
    {
        ...
        var queryBuilder = new Mock<IQueryBuilder<Address>>();
        var projectionMapper = new Mock<IProjectionMapper<Address, AddressProjection>>();
        var expressionBuilder = new ProjectionExpressionBuilder();
        var validator = new AddressSearchQueryValidator();
        var logger = new Mock<ILogger<AddressSearchService>>();

        var service = new AddressSearchService(
            repository.Object,
            queryBuilder.Object,
    ...
    public async Task ExecuteSearchAsync_ReturnsSuccess_WhenQueryValid()
    {
        ...
        var queryable = context.Set<Address>();
        var repository = new Mock<IRepository<Address>>();
        var queryBuilder = new Mock<IQueryBuilder<Address>>();
        var projectionMapper = new Mock<IProjectionMapper<Address, AddressProjection>>();
        var expressionBuilder = new ProjectionExpressionBuilder();
        ...
        Expression<Func<Address, AddressProjection>> projection = entity => new AddressProjection
        {
            Id = entity.Id
        };
