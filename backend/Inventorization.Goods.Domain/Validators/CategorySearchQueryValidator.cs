...
/// <summary>
/// Validates SearchQuery against Category entity metadata.
/// Ensures field names, operators, and types are valid before query execution.
/// </summary>
public class CategorySearchQueryValidator : IValidator<SearchQuery>
{
    ...
    private void ValidateFilterCondition(FilterCondition condition, List<string> errors)
    {
        ...
        {
            errors.Add($"Field '{condition.FieldName}' does not exist in Category entity");
            return;
        }
    ...
    private void ValidateProjection(ProjectionRequest projection, List<string> errors)
    {
        foreach (var field in projection.Fields)
        {
            if (field.IsRelated)
            {
                // Validate related field path (e.g., "ParentCategory.Name")
                ...
                if (!field.RelationPath.Equals("ParentCategory", StringComparison.OrdinalIgnoreCase))
                {
            ...
            else
            {
                ...
                {
                    errors.Add($"Field '{fieldName}' does not exist in Category entity");
                }
            }
        }
    }
    ...
    private void ValidateSort(SortRequest sort, List<string> errors)
    {
        foreach (var field in sort.Fields)
        {
            ...
            {
                errors.Add($"Sort field '{field.FieldName}' does not exist in Category entity");
            }
        }
    }
