...
export class DataAccessGenerator extends BaseGenerator {
  async generate(model: DataModel): Promise<void> {
    ...
    await this.generateDbContext(model, domainProjectPath, namespace, baseNamespace, contextName);
    await this.generateUnitOfWork(model, domainProjectPath, namespace, baseNamespace, contextName);
  ...
  private async generateDbContext(
    ...
    const dbContextsDir = path.join(domainProjectPath, 'DbContexts');
