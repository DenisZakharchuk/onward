...
- � **Generation Stamps** - Each run gets unique 16-char identifier for traceability
- ♻️ **Regeneration Safe** - All files can be regenerated; custom logic in separate services
...
├── Entities/
│   └── {Entity}.cs (fully regeneratable)
├── EntityConfigurations/
│   └── {Entity}Configuration.cs (planned)
├── Creators/
│   └── {Entity}Creator.cs (planned)
├── Modifiers/
│   └── {Entity}Modifier.cs (planned)
├── Mappers/
│   └── {Entity}Mapper.cs (planned)
├── SearchProviders/
│   └── {Entity}SearchProvider.cs (planned)
├── Validators/
│   ├── Create{Entity}Validator.cs (planned)
│   └── Update{Entity}Validator.cs (planned)
├── DataServices/
│   └── {Entity}DataService.cs (planned)
├── DbContexts/
│   └── {Context}DbContext.cs (planned)
├── DataAccess/
│   └── {Context}UnitOfWork.cs (planned)
└── DataModelMetadata.cs (planned)

Inventorization.{Context}.API/
├── Controllers/
│   └── {Entity}sController.cs (planned)
└── Program.cs (planned - DI registrations)
